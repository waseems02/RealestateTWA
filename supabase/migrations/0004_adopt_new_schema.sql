-- ============================================================================
-- 0004_adopt_new_schema.sql
--
-- Adopts Tareq Zahaika's expanded schema (originally on the `database` branch)
-- as the canonical schema for RoomieFit / CampusNest Israel.
--
-- DESTRUCTIVE: drops the existing public.listings / universities / etc. tables
-- and recreates them with new column names (price, source_type, campus_id,
-- listing_type, contact_*, listing_images, profiles, favorites, …). All
-- existing seed rows are replaced by the seed at the bottom of this file
-- (~80 listings across 12 universities / 14 campuses).
--
-- Apply once via the Supabase SQL Editor:
--   https://supabase.com/dashboard/project/<ref>/sql/new
-- After applying, restart the Node server; listingsService normalises the new
-- columns back into the existing API shape so the frontend keeps working.
--
-- All `*_demo` source labels are fake academic records — nothing here was
-- scraped from Yad2, Facebook, or any protected source.
-- ============================================================================
...
create table public.listings (
  ...
  contact_phone text,
  contact_email text,
  source_type text not null default 'manual_demo' check (source_type in ('manual_demo', 'university_board_demo', 'facebook_group_demo', 'yad2_demo', 'public_source_demo', 'manual_user')),
...
-- Expanded fake demo listings for academic use only.
-- Not scraped from Yad2, Facebook, WhatsApp groups, or protected/private sources.
